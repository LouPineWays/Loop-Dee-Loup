...
import { findMatch, endpointsFor, parseArgs, run } from "./poll.mjs";

const BOT = "chatgpt-codex-connector[bot]";
const SINCE = "2026-08-23T14:00:00Z";
const SINCE_MS = new Date(SINCE).getTime();

test("findMatch: matches a genuine post-trigger bot comment", () => {
  ...
  assert.equal(match.endpoint, "issue-comments");
});

test("findMatch: ignores a bot comment from before the trigger time", () => {
  const match = findMatch([{ id: 2, user: { login: BOT }, created_at: "2026-08-23T13:00:00Z" }], {
    bot: BOT,
    sinceMs: SINCE_MS,
...
test("findMatch: ignores a fresh comment from a non-bot author", () => {
  const match = findMatch([{ id: 3, user: { login: "someone-else" }, created_at: "2026-08-23T14:05:00Z" }], {
    bot: BOT,
    sinceMs: SINCE_MS,
    endpointName: "issue-comments",
  });
...
test("findMatch: reviews match via submitted_at instead of created_at", () => {
  const match = findMatch([{ id: 4, user: { login: BOT }, submitted_at: "2026-08-23T14:10:00Z" }], {
    bot: BOT,
    sinceMs: SINCE_MS,
...
test("findMatch: returns the first genuine match, ignoring later items", () => {
  const match = findMatch(
    [
...
test("run: exits 1 on an invalid --since timestamp", async () => {
  const result = await run({ repo: "owner/repo", kind: "issue", number: 53, since: "not-a-date" });
  assert.equal(result.exitCode, 1);
  assert.match(result.message, /not a valid timestamp/);
});

test("run: exits 0 with match JSON the moment a match appears on the first sweep — no sleep call", async () => {
  let sleepCalls = 0;
...
test("run: finds a match only on a later sweep, after sleeping in between", async () => {
  ...
  assert.equal(JSON.parse(result.message).id, 10);
});

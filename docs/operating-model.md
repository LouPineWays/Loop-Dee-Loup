...
- `mode: "fallthrough"` — `NOT_READY` without `postPrLifecycle` only; hands off to the Decomposition boundary / normal reasoning, deliberately unpoliced by this mechanism, matching `AGENTS.md`'s own explicit `NOT_READY` carve-out for every case other than the post-PR exception above.

...
- `mode: "bounded"` — exactly the named `authorizedActions` (e.g. `READY_TO_DISPATCH_PLANNING` -> `["dispatch-planning-worker"]`; `STAGE2_REPORT_READY_TO_RECORD` -> `["run-lifecycle-gate-record-verdict"]`). Some bounded envelopes are context-sensitive rather than a fixed table row: `STAGE2_CLOSE_READY` derives its actual authorized actions from the concrete verdict's own `nextCommand` (a real gated work issue to close first vs. `ACCEPTED_NO_WORK_ISSUE`/an already-closed work issue, audit-only) instead of always offering the superset (Stage 1 finding on PR #534 — the fixed table row alone let the compliance checker accept a work-issue close the concrete verdict never authorized). Bounded (and chain) compliance also enforces that each authorized action occurs at most once and in the envelope's own declared order — set-membership alone previously let a duplicated or reordered action sequence pass as compliant.

...
- `mode: "bounded"` — exactly the named `authorizedActions` (e.g. `READY_TO_DISPATCH_PLANNING` -> `["dispatch-planning-worker", "write-control-snapshot"]`; `STAGE2_REPORT_READY_TO_RECORD` -> `["run-lifecycle-gate-record-verdict"]`).
- `mode: "chain"` — authorizes exactly one further deterministic gate invocation, whose own verdict and envelope then govern what follows (`AUDIT_ISSUE_DETECTED` -> `["run-next-review-transition-gate"]`).
- `mode: "fallthrough"` — `NOT_READY` only; hands off to the Decomposition boundary / normal reasoning, deliberately unpoliced by this mechanism, matching `AGENTS.md`'s own explicit `NOT_READY` carve-out.

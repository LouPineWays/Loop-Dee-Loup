...
1. **`READY_FOR_PLAN`** (Plan). A thick execution Issue exists with no Plan Index yet, and `Route:` is `planning worker`. The gate dispatches that planning worker by reference only (control Issue + execution Issue references — nothing about the execution Issue's content belongs in that dispatch). The worker reads the thick execution Issue in full, determines the complete currently-foreseeable implementation decomposition, establishes any values or interfaces more than one unit depends on, persists the durable plan artifacts below, and returns only a compact `PLAN_READY <ref>`. The controller does not ingest or retransmit the plan body; it records the reference, sets `Lifecycle: PLAN_READY`, and ends.
2. **`PLAN_READY`** (Route/Prepare). A durable `## Execution Plan Index (v1)` comment exists, parseable by `tools/orchestration/parse-execution-plan.mjs`, with no Dispatch Manifest yet. The gate runs `tools/orchestration/prepare-dispatch-manifest.mjs --execution-issue <N> --create` for a first manifest, or the same command with `--comment-id <id>` when updating an existing `## Dispatch Manifest (v1)` comment — a deterministic script invocation applying the cheapest-reliable dispatch tree to the units the plan already defined, not a model-worker dispatch. Stdout alone is not authoritative routing state: before recording `Lifecycle: ROUTED`, the controller must have persisted the manifest durably and verified the resulting comment reference. Only then, and only when no unit lands on `REPLAN_REQUIRED`, may the Lifecycle advance to `ROUTED`; a `REPLAN_REQUIRED` unit is a fail-closed stop, never an authorized transition to `ROUTED`.
3. **`ROUTED`** (Execute). Unit dispatch is authorized only after deterministic durable verification: the Plan Index's `- **Dispatch manifest:**` field must be a non-`none` comment permalink, that exact comment must belong to the same execution Issue, it must carry `## Dispatch Manifest (v1)`, and its `- **Plan index:**` backlink must match the current Plan Index URL. Only then does the gate dispatch every currently `dispatch_ready=true` unit with no unmet prerequisite by reference off that manifest — genuinely independent ready units may be dispatched together as one wave, never a unit with an unmet prerequisite. Each worker reads its own unit contract plus the durable authority it points at, writes durable repository state, and records a compact completion/blocker state on its own contract. Re-evaluate for the next wave once a wave's units are all `DONE` per their own contract's State field, re-read live, never cached; once every plan unit is `DONE`, the Lifecycle transitions to `EXECUTION_COMPLETE`.

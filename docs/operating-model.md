...
2. **`PLAN_READY`** (Route/Prepare). A durable `## Execution Plan Index (v1)` comment exists, parseable by `tools/orchestration/parse-execution-plan.mjs`, with no Dispatch Manifest yet. The gate runs `tools/orchestration/prepare-dispatch-manifest.mjs --execution-issue <N>` — a deterministic script invocation applying the cheapest-reliable dispatch tree to the units the plan already defined, not a model-worker dispatch — and records `Lifecycle: ROUTED` once it completes with no unit landing on `REPLAN_REQUIRED`; a `REPLAN_REQUIRED` unit is a fail-closed stop, never an authorized transition to `ROUTED`.

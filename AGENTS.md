...
- Clean completion: `CLEAN — <what merged/closed>. Next: #<slice>` (or `Next: None`). For a review-worthy PR, never emit this line — and never treat the slice as done in any other way — until `docs/bounded-review-cycle.md` Stage 1 step 9's `tools/review-watch/stage1-gate.mjs` returns `EXEMPT` or `RESPONSE_RECEIVED` from durable GitHub evidence for that PR's frozen head. Completing implementation, tests, commit, push, and PR creation is not itself evidence Stage 1 happened; it is exactly the state in which the omission that produced LDL issue #32 and YouTubery issue #12 occurred, and the gate exists because prose reminders alone did not stop either one.

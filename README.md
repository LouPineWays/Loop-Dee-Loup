...
Loop-Dee-Loup turns a founder-approved feature proposal into small, verified changes while keeping agent sessions disposable and founder interruptions rare. GitHub provides durable state; agents provide execution; repository checks and bounded reviews provide control.
...
- Agent sessions are disposable. The loop continues across them automatically.
- Durable state is a compact snapshot, not a transcript.
- Verification evidence advances the loop. Status prose does not.
- The founder supplies product intent and answers questions that require judgment. Routine technical choices and handoffs remain autonomous.
...
4. Run a fresh executor session for that packet.
5. Verify the result against its acceptance criteria.
...
8. Refresh the parent snapshot and automatically continue until the feature is done or an interrupt condition is reached.
...
Do not request routine permission to create the next packet, implement an accepted approach, run checks, address verified defects, or merge after every required gate passes.
